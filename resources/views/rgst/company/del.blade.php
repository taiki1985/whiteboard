...
@section('content')
  <div class="bl_rgstTable">
    <form action="{{ action('CompanyController@delete')}}" method="post">
      <table>
        <tbody>
            {{ csrf_field() }}
            <input class="id" name="id" type="hidden" value="{{ $form->id }}">
            <tr><th><label for="four_letter">4レター</label></th><td class="four_letter" name="four_letter" type="text">{{ $form->four_letter }}</td></tr>
            <tr><th><label for="company_form">会社形態</label></th><td class="company_form" name="company_form" type="text">{{ $form->company_form }}</td></tr>
            ...
            <tr><th><label for="incharge">担当者</label></th><td class="incharge" name="incharge" type="text">{{ $form->incharge }}</td></tr>
            <tr><th><label for="incharge_phone">担当者連絡先</label></th><td class="incharge_phone" name="incharge_phone" type="text">{{ $form->incharge_phone }}</td></tr>
