...
@section('content')
  <div class="bl_rgstForm_wrapper">
    <div class="bl_rgstForm">
      <form action="{{ action('CompanyController@add')}}" method="post">
        {{ csrf_field() }}
        <label for="four_letter">4レター</label><input class="four_letter" name="four_letter" type="text" value="{{ old('four_letter') }}">
        ...
        <select class="company_form" name="company_form">
          <option value="">----</option>
          <option value="株式会社(前株)" @if(old('company_form')=='株式会社(前株)') selected  @endif>株式会社(前株)</option>
          <option value="株式会社(後株)" @if(old('company_form')=='株式会社(後株)') selected  @endif>株式会社(後株)</option>
        </select>
        ...
        @endif
        <label for="incharge">担当者</label><input class="incharge" name="incharge" type="text" value="{{ old('incharge') }}">
        @if ($errors->has('incharge'))
        <div class="bl_rgstForm_err">{{ $errors->first('incharge') }}</div>
        @endif
        <label for="incharge_phone">担当者連絡先</label><input class="incharge_phone" name="incharge_phone" type="text" value="{{ old('incharge_phone') }}">
