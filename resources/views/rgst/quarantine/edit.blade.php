...
<div class="bl_rgstForm_wrapper">
  <div class="bl_rgstForm">
    <form action="{{ action('QuarantineController@edit')}}" method="post">
      {{ csrf_field() }}
      ...
      @endif
      <label for="jurisdiction">管轄</label><input class="jurisdiction" name="jurisdiction" type="text" value="{{ $form->jurisdiction }}">
      @if ($errors->has('name'))
      ...
      <div class="bl_btnUnit">
        <input class="el_btn" name="submit" type="submit" value="送信"><a class="el_btn" href="{{ action('QuarantineController@index')}}">キャンセル</a>
      </div>
      <!-- /.bl_btnUnit -->
    </form>
  </div>
