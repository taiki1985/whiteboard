...
@section('content')
  <div class="bl_rgstForm_wrapper">
    <div class="bl_rgstForm">
      <form action="{{ action('PortController@add')}}" method="post">
        {{ csrf_field() }}
        ...
        @endif
        <label for="custom_id">税関ID</label><input class="custom_id" name="custom_id" type="number" value="{{ old('custom_id') }}">
        <label for="immigration_id">入管ID</label><input class="immigration_id" name="immigration_id" type="number" value="{{ old('immigration_id') }}">
        <label for="quarantine_id">検疫ID</label><input class="quarantine_id" name="quarantine_id" type="number" value="{{ old('quarantine_id') }}">
