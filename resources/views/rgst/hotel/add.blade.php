...
@section('content')
  <div class="bl_rgstForm_wrapper">
    <div class="bl_rgstForm">
      <form action="{{ action('HotelController@add')}}" method="post">
        {{ csrf_field() }}
        ...
        @endif
        <label for="address">住所</label><textarea class="address" name="address">{{ old('address') }}</textarea>
        @if ($errors->has('address'))
        <div class="bl_rgstForm_err">{{ $errors->first('address') }}</div>
        @endif
        <label for="address_eng">住所(英語)</label><textarea class="address_eng" name="address_eng">{{ old('address_eng') }}"</textarea>
        ...
        @endif
        <label for="web">WEB</label><input class="web" name="web" type="text" value="{{ old('web') }}">
        @if ($errors->has('web'))
        <div class="bl_rgstForm_err">{{ $errors->first('web') }}</div>
        @endif
        <label for="check_in">チェックイン</label><input class="check_in" name="check_in" type="time" value="{{ old('check_in') }}">
        @if ($errors->has('check_in'))
        <div class="bl_rgstForm_err">{{ $errors->first('check_in') }}</div>
        @endif
        <label for="check_out">チェックアウト</label><input class="check_out" name="check_out" type="time" value="{{ old('check_out') }}">
